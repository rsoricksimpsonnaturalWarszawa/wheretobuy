...
'<input type="text" id="openai-chat-input" placeholder='+'"' + 'Wpisz swoje pytanie...' +'"'+ '>' +
...
addChatbotMessage("Cześć! Jestem chatbotem. Jak mogę ci pomóc? (pierwsza wiadomość może zająć kilka sekund)");
...
jQuery(document).ready(function($) {
    ...
    async function handleQuestion(questionElement, answerElement) {
        ...
        $.get('https://raw.githubusercontent.com/medanticancerbelievescotland/knowledge/main/knowledge_pl.txt')
            .then(function(data) {
                var knowledge = data;    
                    ...
                    $.ajax({
                        ...
                        beforeSend: function(xhr) {
                            ...
                            $(answerElement).append('<div class="loading">Pobieranie odpowiedzi...</div>');
                        },
                        success: function(response) {
                            $('.loading').remove();
                            if (response.choices && response.choices[0] && response.choices[0].text) {
                                $(answerElement).append('<div class="question-answer"><strong>Pytanie:</strong> ' + question + '<br><strong>Odpowiedź:</strong> ' + response.choices[0].text + '</div><hr>');
                                ...
                                $(answerElement).append('<div class="error"><strong>Error:</strong> Nie można pobrać odpowiedzi.</div><hr>');
                            }
                        },
                        error: function() {
                            $('.loading').remove();
